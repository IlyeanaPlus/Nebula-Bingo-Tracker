...
function graySample(id,w,h){
  const g=new Uint8Array(w*h);
  for(let i=0,j=0;i<id.data.length;i+=4,j++){
    const r=id.data[i],gn=id.data[i+1],b=id.data[i+2];
    g[j]=(r*299+gn*587+b*114)/1000|0;
  }
  return g;
}
...
export async function computeDHashX(source,size=8){
  ...
  let bits=0,k=0; for(let y=0;y<h;y++) for(let x=0;x<size;x++,k++){const a=g[y*w+x],b=g[y*w+x+1]; if(a>b) bits|=(1<<k)}
  return bits>>>0;
}
...
export async function computeDHashY(source,size=8){
  ...
  let bits=0,k=0; for(let y=0;y<size;y++) for(let x=0;x<w;x++,k++){const a=g[y*w+x],b=g[(y+1)*w+x]; if(a>b) bits|=(1<<k)}
  return bits>>>0;
}
