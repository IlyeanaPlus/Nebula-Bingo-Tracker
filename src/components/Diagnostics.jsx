  ...
  return (
    <section className="mb-6 p-4 bg-white rounded-2xl shadow-sm border border-slate-200">
      ...
      <div className="flex items-center gap-2 mb-2">
        <button disabled={diag.running} className="px-3 py-1 rounded bg-slate-100 hover:bg-slate-200 disabled:opacity-50" onClick={runDiagnostics}>
          {diag.running ? "Running…" : "Run diagnostics"}
        </button>
        <button className="px-3 py-1 rounded bg-slate-100 hover:bg-slate-200" onClick={runHelperTests}>Run helper tests</button>
        <span className="text-xs text-slate-500">Checks API key, list & download.</span>
      </div>
